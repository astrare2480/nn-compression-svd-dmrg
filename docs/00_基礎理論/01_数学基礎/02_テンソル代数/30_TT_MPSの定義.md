...
### $2\times3$ 配列例：reshapeの説明とSVDの制約を分ける

2行3列の値を境界軸付きコアへ移し、全要素の対応を確認する。
ここでは任意の係数配列 $H$ と呼ぶ。
...
一方、この2行3列の配列を3本の正規直交列を持つSVDの $U^{(1)}$ と解釈することはできない。
...
直前の $2\times2$ 実行列例は、このSVDの制約も満たす。reshapeによる要素配置の変更と、SVDによる直交性の制約は別に確認する。
...
「1本の左特異ベクトルの成分数」と「残すベクトルの本数」を分けて考える。
...
左特異ベクトルは $X^{\langle1\rangle}(X^{\langle1\rangle})^T$ の固有ベクトルでもある。
...
### 50サイト例：係数数の増加を実数で確認する

$n=2,d=50$ を、上の式へ代入する。
各サイトの局所状態が2通りなら、全サイトの組合せごとに一つの係数が必要なので、

$$
\begin{aligned}
P_{\mathrm{dense}}
&=\underbrace{2\cdot2\cdots2}_{50\text{ サイト}}\\
&=2^{50}\\
...
これは係数の個数であり、byte単位の容量ではない。
一方、内部bondを全て $r$ とするTTの保存要素数は、境界の $r_0=r_{50}=1$ も入れて

$$
\begin{aligned}
P_{\mathrm{TT}}
&=1\cdot2\cdot r
+\sum_{k=2}^{49}r\cdot2\cdot r
+r\cdot2\cdot1\\
&=2r+48\cdot2r^2+2r\\
&=4r+96r^2.
\end{aligned}
...
となる。ただし、任意の50サイト状態を小さい $r$ で厳密に表せるという意味ではない。
必要なbondは各cutのSchmidt rankに依存し、より小さく保持するなら近似誤差が生じる。
rankを固定したときの保存量と、所望の精度を得るために必要なrankは分けて考える。
Tuckerとの同じshape条件での具体計算は [[15_TuckerとTTの比較]] に集約する。
