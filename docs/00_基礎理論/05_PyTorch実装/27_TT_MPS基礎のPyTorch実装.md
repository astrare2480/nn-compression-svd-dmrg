...
第1 SVDをexactに行った場合でも、第2段階のremainderは元の物理座標 $(i_1,i_2,i_3)$ の配列ではない。
左側の $i_1$ は既に第1コアへ移り、remainderにはその保持基底での係数ラベル $\alpha_1$ が入る。
従って次のSVDに渡す行は $(\alpha_1,i_2)$ であり、元Tensorの第2cutの行 $(i_1,i_2)$ とは別の座標である。
exactの場合に両者のrankが一致するのは、[[34_基底変換とTT-rank不変性]] の等長写像でつながっているからであって、二つの入力行列の値やshapeが同じだからではない。
...
### shapeだけでなく、縮約される要素も書く

最初の縮約で残る4本の軸を区別すると

$$
\begin{aligned}
T_{1,i_1,i_2,\alpha_2}
&=\sum_{\alpha_1=1}^{r_1}
G^{(1)}_{1,i_1,\alpha_1}G^{(2)}_{\alpha_1,i_2,\alpha_2},\\
Z_{1,i_1,i_2,i_3,1}
&=\sum_{\alpha_2=1}^{r_2}
T_{1,i_1,i_2,\alpha_2}G^{(3)}_{\alpha_2,i_3,1}\\
&=\sum_{\alpha_2=1}^{r_2}\sum_{\alpha_1=1}^{r_1}
G^{(1)}_{1,i_1,\alpha_1}
...
ここで dims=([-1], [0]) は、左の最後のbond軸と右の最初のbond軸を同じ添字で足す指定である。
縮約しなかった左の軸が先、右の軸が後に残るので、
第1回は $(1,n_1,n_2,r_2)$、第2回は $(1,n_1,n_2,n_3,1)$ となる。

例えば $r_1=r_2=2$、physical indexを固定し
$g_\alpha:=G^{(1)}_{1,i_1,\alpha}$、
$h_{\alpha,\beta}:=G^{(2)}_{\alpha,i_2,\beta}$、
$q_\beta:=G^{(3)}_{\beta,i_3,1}$ と置けば

$$
\begin{aligned}
T_{1,i_1,i_2,1}&=g_1h_{1,1}+g_2h_{2,1},\\
T_{1,i_1,i_2,2}&=g_1h_{1,2}+g_2h_{2,2},\\
X_{i_1,i_2,i_3}
&=(g_1h_{1,1}+g_2h_{2,1})q_1
+(g_1h_{1,2}+g_2h_{2,2})q_2\\
&=g_1h_{1,1}q_1+g_2h_{2,1}q_1
+g_1h_{1,2}q_2+g_2h_{2,2}q_2.
\end{aligned}
$$

最後のサイズ1の境界軸だけを取り除けば、元のphysical index順のテンソルが得られる。
