...
例えば $(r_1,n_2,r_2)=(3,4,5)$ なら $N=3\cdot4\cdot5=60$ なので、

$$
\sqrt{\mathbb E\|\Delta C\|_F^2}
=10^{-3}\sqrt{60}
\approx 7.75\times10^{-3}.
$$

各要素の標準偏差は $10^{-3}$ でも、60要素を合わせたノルムの尺度は約 $7.75\times10^{-3}$ となる。これは分布から計算した尺度であり、乱数を1回生成したときに必ずこの値になるという意味ではない。
