...
- `TensorErrorMetrics`、`compare_tensors`：[[00_基礎理論/05_PyTorch実装/07_PyTorch実装#15. rankを下げた出力誤差]]
- `WeightErrorMetrics`、`compare_weight_matrices`：[[00_基礎理論/05_PyTorch実装/07_PyTorch実装#16. 重み誤差]]
...
補足：上の `compare_tensors` がcosine similarityも「まとめる」という記載は、元教材の戻り値との対応を述べたものである。現行の第3節の `TensorErrorMetrics` と `compare_tensors` にはcosineのfield・計算が含まれていないため、現行コードを実行するときには別指標として計算する。元教材の零ベクトル除外・有効行だけの平均・有効行がない場合の `None` を回収した処理と全要素の確認例は [[07_PyTorch実装#元教材のコサイン類似度平均を別の指標として補う]] に置いた。関数・データクラスの定義本体は [[07_PyTorch実装#3. 実装全体]]、第15・16節は使用例である。
...
`compare_tensors` の完成実装は [[00_基礎理論/05_PyTorch実装/07_PyTorch実装#15. rankを下げた出力誤差]] を参照する。ここでは、圧縮前後で入力と評価条件をそろえることが要点である。
