...
第4成分を一つ削る損失は $0.0001$、第1成分なら $100$ である。これはFrobeniusノルムで測った大きさの比較であり、個別の観測量や長距離相関への影響を同じ比率で保証するものではない。

ここまでで導いたのは、**このSchmidt展開から末尾を捨てたときの**誤差である。さらにEckart–Young–Mirsky定理を用いると、すべてのrankが高々 $k$ の行列 $B$ の中でも
...
が成り立つ。最初の等号はこのノートの直交性計算だけからは出ず、定理による追加の主張である。最良低rank近似の証明スケッチは [[00_基礎理論/01_数学基礎/01_線形代数/02_SVD数式の導出]]、複数cutへの適用は [[33_TT-SVDの打ち切りと誤差]] を参照する。
