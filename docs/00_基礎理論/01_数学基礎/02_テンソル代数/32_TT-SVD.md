...
$$
X_{i_1,i_2,i_3}
=U^{(1)}_{i_1,1}\mathcal B^{(1)}_{1,i_2,i_3}
+U^{(1)}_{i_1,2}\mathcal B^{(1)}_{2,i_2,i_3}.
$$

となる。$\mathcal B^{(1)}_{1,:,:}$ と $\mathcal B^{(1)}_{2,:,:}$ は、
どちらも $n_2\times n_3$ の係数配列だが、一般には値の異なる別のスライスである。
前者は $U^{(1)}_{:,1}$、後者は $U^{(1)}_{:,2}$ と対をなす。
この番号を忘れて二つを混ぜると、どの左基底にどの右側係数を掛けるかが失われる。

実係数の状態として読む場合、右特異ベクトルから

$$
\begin{aligned}
|L_{\alpha_1}\rangle
&=\sum_{i_1=1}^{n_1}U^{(1)}_{i_1,\alpha_1}|i_1\rangle,\\
|R_{\alpha_1}\rangle
&=\sum_{i_2=1}^{n_2}\sum_{i_3=1}^{n_3}
V^{(1)T}_{\alpha_1,(i_2,i_3)}|i_2,i_3\rangle,\\
\mathcal B^{(1)}_{\alpha_1,i_2,i_3}
&=\sigma_{\alpha_1}^{(1)}
\langle i_2,i_3|R_{\alpha_1}\rangle.
\end{aligned}
$$

と対応する。したがって二つのスライスを含む再構成は、

$$
|\Psi\rangle
=\sigma_1^{(1)}|L_1\rangle\otimes|R_1\rangle
+\sigma_2^{(1)}|L_2\rangle\otimes|R_2\rangle.
$$

である。remainderのスライスは、規格化された $|R_{\alpha_1}\rangle$ の係数だけではなく、
その重み $\sigma_{\alpha_1}^{(1)}$ も含む。
この節ではexact rankが2の場合を扱った。保持bondを2に打ち切る場合には、左辺は保持された近似である。
複素係数での共役とSchmidt対応の導出は [[30_TT_MPSの定義]] を参照する。
