...
このため、「特異値energy」は $\sigma_i$ の単純和ではなく二乗和で測る。
...
> [!note] 補足導出
> truncated SVDは固定rankで最良近似になるが、最適解は一意とは限らない。このため `∈ argmin` と表す。証明スケッチは [[00_基礎理論/01_数学基礎/01_線形代数/02_SVD数式の導出]] に置く。
...
> [!note] 補足導出
> 入力分布を含むtrace式は、weight誤差と実データoutput誤差が別の量であることを示す。
...
### なぜ低rankを期待するかと、保証しないこと

学習済み重みが必ず低rankになるとは限らない。低rank近似を試す理由として、隠れユニットが似た特徴を学ぶこと、入力が高次元空間の一部へ集中すること、モデルに余分な幅があること、複数出力が特徴を共有することが挙げられる。また、最適化で作用が一部の方向へ集中したり、正則化で構造が単純になったり、学習データに現れない方向が十分使われなかったりする可能性もある。
...
小さい特異値の方向にも、分類境界の微調整、特定クラスの識別、まれな入力パターン、後続層との組合せに必要な情報がある可能性がある。従ってスペクトルの折れ曲がりはrank候補であって、出力誤差・validation性能を確認しない最終決定ではない。層によって必要なrankが異なり、小さい層では2層化の利点が小さいことにも注意する。
...
rankの選び方には、固定rank、energy閾値、スペクトルの折れ曲がり、目標パラメータ数、validationで許容できる精度低下、fine-tuning後の性能がある。圧縮直後のSVDは行列ノルムを基準にしているが、fine-tuningは入力分布・後続層・損失・分類境界に合わせ、低rankの因子をタスクへ再適応させる。因子の中間幅 $r$ を変えない限り、再学習しても合成重みのrank上限は $r$ のままである。各選び方と最終評価の分離は [[05_圧縮率とRank]]、[[10_SVD圧縮モデルの評価設計]]、因子の学習とrank制約は [[04_Linear層を2層へ置き換える#44. fine-tuning後もrank制約が保たれる理由]] に対応する。

## 19. 実装との対応

PyTorchのReduced SVDは

PyTorchの確認コード：[[05_SVD基礎実装検証/00_SVD理論のPyTorch確認コード#PyTorch確認-007]]

rank$r$なら

PyTorchの確認コード：[[05_SVD基礎実装検証/00_SVD理論のPyTorch確認コード#PyTorch確認-008]]

である。

再構成は

PyTorchの確認コード：[[05_SVD基礎実装検証/00_SVD理論のPyTorch確認コード#PyTorch確認-009]]
...
## 19.1 画像ごとのSVD・画像集合のPCA・NN重みのSVDを区別する

同じSVDでも、何を行列にするかで目的が異なる。

- 画像ごとのSVD：1画像の画素行列 $X\in\mathbb R^{28\times28}$ を近似する。
- 画像集合のPCA：各画像を784次元vectorへして、$D\in\mathbb R^{N\times784}$ を中心化して分解する。
- NN重みのSVD：学習済みweight $W\in\mathbb R^{m\times n}$ を2因子へ置換する。
...
として特異値だけを分類特徴にする使い方がある。後者は左右特異vectorを捨てるため、特異値だけから元画像を復元できない。
...
従って両者の特異値は $(3,1)$ だが、画素配置は異なる。rank 1近似は
...
特異値だけを入力する分類器は、この2画像を区別できない。

### 同じ2画像を、画像集合のPCAとして計算する

画素を行優先で並べ、1画像を1行にすると、
...
より0と10であり、非零特異値は $\sqrt{10}$ である。対応する左右特異vectorを
...
$$
D_c=\sqrt{10}\,uv^{\mathsf T},
\qquad
D_cv=
\begin{pmatrix}\sqrt5\\-\sqrt5\end{pmatrix}.
$$
...
\end{pmatrix}\\
&=
\begin{pmatrix}3&0&0&1\\0&1&3&0\end{pmatrix}
=D.
\end{aligned}
$$

画像ごとのrank 1近似で誤差が残ったことと矛盾しない。前者は各 $2\times2$ 画素行列、後者は中心化した $2\times4$ の画像集合行列のrankだからである。

特異値特徴を分類へ使う場合は、train画像から得た特徴列でScalerをfitし、validation/testには同じ平均とscaleを使う。NN重み圧縮では画像を特異値特徴へ置き換えず、学習済み $W$ をSVDし、因子を層へ格納して同じ入力で評価する。
...
- PyTorchでの重み分解：[[05_SVD基礎実装検証/08_Linear層のSVD実装]]
