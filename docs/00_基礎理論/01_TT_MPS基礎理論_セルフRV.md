...
> 2026-09-12：本RVの12ファイルと後続範囲の記述は、上記の元資料を整理した時点の評価として保持する。新しく添付された左右QRの資料と36〜39の追加は [[02_TT_MPS正準形_資料対応監査]] に分けて記録する。今回、既存Notebook/srcの再実行・再レビューはしていない。
...
このcoverageは理論論点・結論・必要な導出やindex対応の反映を評価する。一方、具体行列や全要素表示をPerplexity資料と同じ細かさで示しているかは**説明粒度**の評価であり、簡略化されていることだけをcoverage gapとは扱わない。
...
ただし、これは理論論点のcoverage判定であり、具体行列や全要素表示の説明粒度まで元資料と同等であるという判定ではない。
