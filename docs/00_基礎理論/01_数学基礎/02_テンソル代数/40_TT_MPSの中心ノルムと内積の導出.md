...
添付 `canonical_form.md` の610–1474行、1793–2485行にある、
中心コアの座標としての意味、ノルム・差分・内積の導出と非正準形との比較を回収する。
元資料の有限和の並べ替え、独立した二組のボンド添字、Gramからデルタへの置換、
デルタで和を消す手順を省略しない。日本語は教材向けに整理している。
...
元資料1523–1698行のコアunfoldingの条件も、行列の略記へ対応させる。
...
元資料775–834行と1415–1474行の比較を、同じ和の順序で確認する。
左右が直交化されていない表現を
...
元資料1235–1414行では左右を固定し、中心だけを $C+\Delta C$ へ変えている。
...
元資料1415–1474行の差分に対する式を、$C$ の場合への参照だけでなく書くと
...
元資料2139–2399行を回収する。左右は**同じ** $L,R$、中心だけを $C,D$ とする。
...
元資料の相互Gramの説明を、ここでは最終的な和まで展開した。
...
本節だけは元資料からの転載ではなく、導出に対応させた独自の補足例である。
...
元資料1699–1787行の一般 $d$ 階の条件は、中心 $c$ より左を左直交、
...
元資料835–918行にある実用上の利点は、規格化の確認、中心だけを変えた誤差の測定、
...
元資料2486–2633行で次段階として予告する局所最適化・中心移動は、本章でも未実装である。
理論の回収と、ユーザー本人によるNotebook実装・理解の完了を混同しない。

## 9. 元資料の予告と演習範囲も区別して残す

元資料2486–2560行が挙げた次段階は、中心の意味、中心だけの最小二乗問題、
QR/SVDによる中心移動、Schmidt分解・roundingとの関係、DMRGへの接続である。
DMRGへの接続はユーザーの指定により今回対象外とし、その説明・導出は追加しない。
これは詳細な導出を済ませた章ではなく、次の学習への予告である。
元資料の $G_2^{[C]}=Q_2R_2$ という式も、3階コアそのものの通常の行列QRではない。
左unfoldingして
...
元資料の「唯一の非直交部分」は、中心以外に片側の直交条件を課すという意味であり、
中心がたまたま直交条件を満たす可能性を否定するものではない。

元資料2561–2633行の演習依頼は、実装なしで一問一論点を確認する次の順である。
...
元資料は問題を先に出して解答を後から添削し、物理的解釈の詳説・中心最適化・中心移動・
SVD/Schmidt・rounding・MPO/TT-matrix・DMRGの問題は出さないという区切りだった。
